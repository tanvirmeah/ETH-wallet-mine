...
async def balance(addr, session):
    ...
    async with session.get(url) as response:
        if response.status == 200:
            ...
            if "data" in data and "total_wallet_balance" in data["data"]:
                balance_eth = float(data["data"]["total_wallet_balance"])
    ...
    async with session.get(url) as response:
        if response.status == 200:
            ...
            if "data" in data and "assets" in data["data"]:
                txs = sum(asset["token_balance"] for asset in data["data"]["assets"])
        ...
        for word in words:
            ...
            try:
                ...
                panel_content = (
                    ...
                    f"[ad00ff]Total Balance: [{total_balance_color}]{total_balance:>20.16f} BNB[/]\n"
                    f"[b magenta]  [/]\n"
                    f"[ad00ff]Address: [fff]{addr}\n"
                    f"Mnemonic: {mnemonic_words}\n"
                    f"Balance: [green]{bal:.16f} BNB (Updated: {bal_time})[/]\n"
