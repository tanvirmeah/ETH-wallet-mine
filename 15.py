...
async def main():
    ...
    if len(words) < 2048:
        raise ValueError("Insufficient words in words.txt. Expected at least 2048 words.")

    async with aiohttp.ClientSession() as session:
        Account.enable_unaudited_hdwallet_features()  # Enable Mnemonic features
        while True:
            try:
                # Generate new 15-word mnemonic passphrase
                random.shuffle(words)
                words_split = ' '.join(words[:15])  # Take the first 15 shuffled words
